import { Heading, Spinner, VStack, HStack, SimpleGrid, GridItem, useBreakpointValue, FormControl, FormLabel, Input, Box, Tag, Button, ButtonGroup } from "@chakra-ui/react"
...
const Explore = ({ categories, text }) => {
    ...
    useEffect(() => {
        ...
        if (!selectedCatagory && !serachText) return setFilterdGiveaways(allGiveaways)

        if (selectedCatagory && !serachText) {
            filterProcess = filterProcess.filter(item => item.category.name === selectedCatagory)
            setFilterdGiveaways(filterProcess)
        }

        if (!selectedCatagory && serachText) {
            filterProcess = filterProcess.filter(item => item.name.includes(serachText))
            setFilterdGiveaways(filterProcess)
        }
    ...
    return (
        <VStack w={'100%'}>
            ...
            {allGiveaways ?
                <HStack>
                    <SimpleGrid columns={8} w={'full'} rowGap={5} columnGap={5}>
                        {filtedGiveaways ?
